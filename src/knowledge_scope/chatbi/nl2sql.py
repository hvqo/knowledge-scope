...
def _build_sql_only_messages(request: _NL2SQLRequest) -> list[LLMMessage]:
    """Build the bounded A5.7j2 SQL-only prompt experiment."""
    system = (
        f"KnowledgeScope NL2SQL contract {NL2SQL_PROMPT_VERSION} (SQL-only).\n"
        ...
        "Use explicit joins supported by the context and keep the query bounded. Before writing "
        "SQL, internally determine the requested visible fields, dimensions, metrics, row grain, "
        "filters, date boundaries, ordering, and top-k semantics from the question.\n"
        "Every field explicitly requested by the user must remain visible in the final SELECT "
        "unless SQL semantics make that impossible; never silently replace a requested display "
        "field with an internal code or ID. Distinguish dimensions that identify or group entities "
        "from metrics such as counts, sums, averages, or derived values, and return both when both "
        "are requested.\n"
        "Choose the intended row grain before constructing SQL: one total, one entity, one group, "
        "one detail record, or one entity pair. For entity-level grouping, use stable identity in "
        "GROUP BY when needed to keep distinct entities separate, but do not expose internal IDs "
        "in detail results unless explicitly requested or needed to distinguish user-visible "
        "entities.\n"
        "Keep the exact requested metric semantics. Do not replace a field-specific count with "
        "COUNT(*) when that changes the requested meaning. Every non-aggregated requested "
        "dimension "
        "required by the row grain must be represented consistently in SELECT and GROUP BY.\n"
        "Preserve explicitly requested filters, date boundaries, ordering, and ranking/top-k "
        "semantics. Safety LIMIT behavior must not redefine those requested semantics. Select only "
        "the columns or derived values needed to answer the question; do not add helpful, "
        "descriptive, identifier, date, region, customer, or metadata columns unless requested, "
        "required for user-visible disambiguation, or semantically necessary.\n"
